...
func TestWalletRepository_HasSufficientBalance(t *testing.T) {
	tests := map[string]struct {
		prepareDB      func() (*WalletRepository, *uuid.UUID)
		inputAmount    float64
		expectedResult bool
		...
		"should return true when balance + amount >= 0 (positive result)": {
			prepareDB: func() (*WalletRepository, *uuid.UUID) {
				db := setupWalletTestDB()
				repo := NewWalletRepository(db)

				walletMock := fixture.WalletMock(
					fixture.WithWalletBalance(1000.0), // Balance: 1000
				)
				walletDB := FromWalletDomain(walletMock)
			...
			inputAmount:    -500.0, // 1000 + (-500) = 500 >= 0
			expectedResult: true,
			expectedErr:    nil,
		},
		"should return true when balance + amount = 0 (exact zero)": {
			prepareDB: func() (*WalletRepository, *uuid.UUID) {
				db := setupWalletTestDB()
				repo := NewWalletRepository(db)

				walletMock := fixture.WalletMock(
					fixture.WithWalletBalance(500.0), // Balance: 500
				)
				walletDB := FromWalletDomain(walletMock)
			...
			inputAmount:    -500.0, // 500 + (-500) = 0 >= 0
			expectedResult: true,
			expectedErr:    nil,
		},
		"should return false when balance + amount < 0 (insufficient balance)": {
			prepareDB: func() (*WalletRepository, *uuid.UUID) {
				db := setupWalletTestDB()
				repo := NewWalletRepository(db)

				walletMock := fixture.WalletMock(
					fixture.WithWalletBalance(300.0), // Balance: 300
				)
				walletDB := FromWalletDomain(walletMock)
			...
			inputAmount:    -500.0, // 300 + (-500) = -200 < 0
			expectedResult: false,
			expectedErr:    nil,
		},
		"should return true when adding positive amount (income)": {
			prepareDB: func() (*WalletRepository, *uuid.UUID) {
				db := setupWalletTestDB()
				repo := NewWalletRepository(db)

				walletMock := fixture.WalletMock(
					fixture.WithWalletBalance(100.0), // Balance: 100
				)
				walletDB := FromWalletDomain(walletMock)
			...
			inputAmount:    200.0, // 100 + 200 = 300 >= 0
			expectedResult: true,
			expectedErr:    nil,
		},
		"should return error when wallet not found": {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			repo, walletID := tc.prepareDB()
			ctx := context.WithValue(context.Background(), authentication.UserID, "user-test-id")

			result, err := repo.HasSufficientBalance(ctx, walletID, tc.inputAmount)
