...
func TestPgRepository_Add(t *testing.T) {
	...
	}{
		{
			...
			mockFunc: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(
					`INSERT INTO "type_payments" ("description","date_create","date_update") VALUES ($1,$2,$3) RETURNING "id"`)).
					WillReturnRows(sqlmock.NewRows([]string{"id", "description", "date_create", "date_update"}).
						AddRow(typePaymentsMock[0].ID, typePaymentsMock[0].Description, typePaymentsMock[0].DateCreate, typePaymentsMock[0].DateUpdate))
				return db, mock, err
			},
		},
		{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := repo.Add(context.Background(), tc.inputTypePayment)
	...
	}{
		{
			...
			mockFunc: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(
					...
					WillReturnRows(sqlmock.NewRows([]string{"id", "description", "date_create", "date_update"}).
						AddRow(typePaymentsMock[0].ID, typePaymentsMock[0].Description, typePaymentsMock[0].DateCreate, typePaymentsMock[0].DateUpdate).
						AddRow(typePaymentsMock[1].ID, typePaymentsMock[1].Description, typePaymentsMock[1].DateCreate, typePaymentsMock[1].DateUpdate).
						AddRow(typePaymentsMock[2].ID, typePaymentsMock[2].Description, typePaymentsMock[2].DateCreate, typePaymentsMock[2].DateUpdate))
				return db, mock, err
			},
		},
		{
...
func TestPgRepository_Update(t *testing.T) {
	...
	}{
		{
			...
			mockFunc: func() (*sql.DB, sqlmock.Sqlmock, error) {
				...
				mock.ExpectExec(regexp.QuoteMeta(
					`UPDATE "type_payments" SET "description"=$1,"date_create"=$2,"date_update"=$3
							WHERE "id" = $4`)).
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := repo.Update(context.Background(), 2, tc.inputTypePayment)
			require.Equal(t, tc.expectedErr, err)
