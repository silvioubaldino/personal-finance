...
func (s *Mock) FindByID(_ context.Context, id int) (model.Movement, error) {
	args := s.Called(id)
	return args.Get(0).(model.Movement), args.Error(1)
}

func (s *Mock) FindByMonth(_ context.Context, _ model.Period) ([]model.Movement, error) {
	args := s.Called()
	return args.Get(0).([]model.Movement), args.Error(1)
}
...
func (s *Mock) Update(_ context.Context, _ int, transaction model.Movement) (model.Movement, error) {
	args := s.Called(transaction)
	return args.Get(0).(model.Movement), args.Error(1)
}
